  ...
  @gradients: (coords) ->
    ...
    for coord, i in coords
      if coord.y?
        ...
        prManageroord = coords[i - 1] or {y: null}
        if prManageroord.y? and nextCoord.y?
          grad(prManageroord, nextCoord)
        else if prManageroord.y?
          grad(prManageroord, coord)
        else if nextCoord.y?
